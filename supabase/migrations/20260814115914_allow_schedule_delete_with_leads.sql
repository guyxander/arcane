...
begin
  ...
  if p_action = 'update_slot' then
    ...
    if not exists(select 1 from public.availability_slots where id = sid) then
      raise exception 'Schedule not found';
    end if;
    ...
    set slot_id = null,
        status = case when status = 'schedule_review' then 'new' else status end,
        updated_at = now()
    where slot_id = sid;
    ...
    values(
      ...
      'availability_slot',
      sid::text,
      jsonb_build_object('deleted', true, 'detached_enrollments', linked_enrollments)
    );

    return jsonb_build_object(
      'ok', true,
      'id', sid,
      'detached_enrollments', linked_enrollments
    );
  end if;
